...
你可能已经注意到，到目前为止的示例都没有使用引号。我们的目标是让 D2 易于使用，而引号往往会带来不便。

引号在多个方面造成不便。首先，你必须闭合已打开的引号。其次，你必须记住是使用单引号还是双引号。最后，它们会增加语法噪音。
...
自动格式化程序会检查是否存在缩进不足的非空行。如果是，所有行都会被添加正确的缩进量（同时保留现有的公共缩进量）。这样你就不必在编辑器中调整缩进了。只需在块字符串起始后的第一列粘贴任何代码，自动格式化程序就会为你纠正。

在缩进至基础块字符串缩进（两个空格）后，你可以使用制表符来缩进块字符串。基础块字符串缩进中的任何制表符都会被自动转换为两个空格。
